...
class ResearcherAgent:
    ...
    async def gather_all_evidence(self, query: str) -> List[Dict[str, Any]]:
        """
        1) Qdrant semantic seeds + rerank
        2) Graph expand (owner spans + references + semantics + mention spans)
        3) Fusion scoring rồi hợp nhất thành list context (để cho Answer Agent)
        """
        # 1) Qdrant + rerank
